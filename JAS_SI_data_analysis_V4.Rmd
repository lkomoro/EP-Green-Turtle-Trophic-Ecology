...
ggplot(data_new2, aes(x=d13C,y=d15N)) +
  stat_ellipse(geom="polygon",alpha=.5, aes(fill=Location_Label),color="black")+ geom_point(size=3, alpha=.9,shape=21,position=position_jitter(width=.5,height=.5),aes(fill=Location_Label))+theme_bw()+theme(axis.text.x  = element_text(angle=0, vjust=0.5))+facet_wrap( ~ Region, ncol=2)
dev.off()

pdf("ellipse_facet2.pdf", 8, 5)
#with data points and ellipse for each site by region-points on bottom
ggplot(data_new2, aes(x=d13C,y=d15N)) +
   geom_point(size=3, alpha=.9,shape=21,position=position_jitter(width=.5,height=.5),aes(fill=Location_Label))+theme_bw()+theme(axis.text.x  = element_text(angle=0, vjust=0.5))+
stat_ellipse(geom="polygon",alpha=.5, aes(fill=Location_Label),color="black")+facet_wrap( ~ Region, ncol=2)
dev.off()
...
pdf(paste(description1, "_map", ".pdf",sep=""), width = 8, height = 8.5)
ggmap(myMap1) +
  geom_point(aes(x = Longitude, y = Latitude), 
             data = EPGT_latlong,fill = "red", alpha=0.65, 
             size = EPGT_latlong$mean_d15N, shape=21)
dev.off()

#Cen-Islands_inset
description2<-"Islands_region"
Islands_region <- c(-95, -10, -75, 11)
#bounding box lowerleftlon, lowerleftlat, upperrightlon, upperrightlat

myMap2 <- get_map(location=Islands_region, 
                  source="stamen", maptype="watercolor", crop=TRUE) 

pdf(paste(description2, "_map", ".pdf",sep=""), width = 8, height = 8.5)
ggmap(myMap2) +
  geom_point(aes(x = Longitude, y = Latitude), 
             data = EPGT_latlong, fill = "red", alpha=0.65,  
             size = EPGT_latlong$mean_d15N, shape=21)
...
pdf(paste(description3, "_map", ".pdf",sep=""), width = 8, height = 8.5)
ggmap(myMap3) +
  geom_point(aes(x = Longitude, y = Latitude), 
             data = EPGT_latlong, fill = "red", alpha=0.65, 
             size = EPGT_latlong$mean_d15N,shape=21)
...
SC_Cm1<-aov(data=SC, d15N~Ordered_SITE*CCL_combined)
GC_Cm1<-aov(data=GC, d15N~Ordered_SITE*CCL_combined)
CSC_Cm1<-aov(data=CSC, d15N~Ordered_SITE*CCL_combined)
EPI_Cm1<-aov(data=EPI, d15N~Ordered_SITE*CCL_combined)
...
Lisa to do in am:
1. take output from each model and put into tables, and make appropriate graphs for each region (i.e., if site x size significant, should be seperate regression lines; if just main effects are signifcant should be one slope, etc.) paste in power point so can walk through results with Jeff of findings
-run tukey for if/when only site is significant and add to slides too

2. Ask him about ellipses graphs and map formatting

3. Ask him about if wants to do analyses looking at significant differences in variation
