...
####7. CSIA graphic
```{r CSIA, echo=TRUE,include=TRUE }
library(tidyr)
library(dplyr)
CSIA<-read.csv("CSIA Raw data.csv")
...
df <- add_rownames(meanN, "amino.acid")
...
pdf("CSIA_dotplot.pdf", 8, 5)
g1+scale_shape_manual(values=shape,name="Study Site")+scale_fill_manual(values=fill,name="Study Site")+geom_vline(xintercept = 7.5, linetype = "longdash",color="grey30")
dev.off()
```
