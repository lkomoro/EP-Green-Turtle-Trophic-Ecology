...
```{r hier_models, echo=TRUE,include=TRUE }
...
#nested fixed effects ANOVA-(just running to explore but likely not useful)
N_M5<-gls(d15N~Region/Ordered_SITE, data=data)
summary(N_M5)
par(mfrow=c(2, 1))
plot(N_M5, which=1:2)
dev.off()
data$N_M5resid = resid(N_M5)
ggplot(data,aes(x=Ordered_SITE,y=N_M5resid))+geom_boxplot()+geom_point()#problem with heteroscedasticity; and appears that gls varIdent can't run with nested fixed effects models
#vf1<-varIdent(form=~1|Ordered_SITE) 
#N_M5.1<-gls(d15N~Region/Ordered_SITE, weights=vf1,data=data) #error message:Error in glsEstimate(object, control = control) : 
 # computed "gls" fit is singular, rank 17
...
sjt.lmer(N_M1.1,N_M1.3)#Error for tables: perhaps sjPlots only works with lme4 package lmer() mixed models, and only gls() function for nlme? come back to

vf<-read.csv("N_M1.1_variancefactors.csv")
...
#nested fixed effects ANOVA-(just running to explore but likely not useful)
C_M5<-lm(d13C~Region/Ordered_SITE, data=data)
summary(C_M5)
par(mfrow=c(2, 1))
plot(C_M5, which=1:2)
dev.off()
data$C_M5resid = resid(C_M5)
ggplot(data,aes(x=Ordered_SITE,y=C_M5resid))+geom_boxplot()+geom_point()#problem with heteroscedasticity; and appears that gls varIdent can't run with nested fixed effects models
#vf1<-varIdent(form=~1|Ordered_SITE) 
#C_M5.1<-gls(d13C~Region/Ordered_SITE, weights=vf1,data=data) #error message:Error in glsEstimate(object, control = control) : 
 # computed "gls" fit is singular, rank 17
...
vfC<-read.csv("C_M1.1_variancefactors.csv")
...
#### Linear Models   (now just anovas bc took out turtle body size due to missing data from some sites)  
(Just left these in from exploratory DA in case turns out useful to look at fanning, etc.)  
